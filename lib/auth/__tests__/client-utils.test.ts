...
describe('Client Auth Utilities', () => {
  describe('getRedirectUrl', () => {
    it('should return the next parameter when it is a valid relative URL', () => {
      ...
      expect(result).toBe('/read/test-doc')
    })
    ...
    it('should accept deep relative paths', () => {
      const searchParams = new URLSearchParams('next=/read/test-doc/tweets')
      const result = getRedirectUrl(searchParams)

      expect(result).toBe('/read/test-doc/tweets')
    })

    it('should accept paths with query parameters', () => {
      const searchParams = new URLSearchParams('next=/read/test-doc?tab=summary')
      const result = getRedirectUrl(searchParams)

      expect(result).toBe('/read/test-doc?tab=summary')
    })

    it('should accept paths with fragments', () => {
      const searchParams = new URLSearchParams('next=/read/test-doc#section-1')
      const result = getRedirectUrl(searchParams)

      expect(result).toBe('/read/test-doc#section-1')
    })
    ...
    describe('Edge cases', () => {
      ...
      it('should handle null bytes and special characters', () => {
        const searchParams = new URLSearchParams('next=/read/test\x00doc')
        const result = getRedirectUrl(searchParams)

        expect(result).toBe('/read/test\x00doc') // Browser will handle null bytes
      })

      it('should handle very long paths', () => {
        const longPath = '/read/' + 'a'.repeat(1000)
    ...
    describe('Multiple query parameters', () => {
      it('should extract next parameter when other parameters are present', () => {
        const searchParams = new URLSearchParams('utm_source=google&next=/read/test-doc&utm_medium=cpc')
        const result = getRedirectUrl(searchParams)
      ...
      it('should handle multiple next parameters by using the first one', () => {
        const searchParams = new URLSearchParams('next=/read/first&next=/read/second')
        const result = getRedirectUrl(searchParams)

        expect(result).toBe('/read/first')
      })

      it('should handle case sensitivity of parameter names', () => {
        const searchParams = new URLSearchParams('Next=/read/test-doc&NEXT=/read/other')
        const result = getRedirectUrl(searchParams)
    ...
    describe('Real-world scenarios', () => {
      it('should handle login redirect from document page', () => {
        const searchParams = new URLSearchParams('next=/read/chalmers-consciousness-paper')
        const result = getRedirectUrl(searchParams, '/dashboard')

        expect(result).toBe('/read/chalmers-consciousness-paper')
      })

      it('should handle login redirect from tweet thread page', () => {
        const searchParams = new URLSearchParams('next=/read/chalmers-consciousness-paper/tweets')
        const result = getRedirectUrl(searchParams, '/dashboard')

        expect(result).toBe('/read/chalmers-consciousness-paper/tweets')
      })
      ...
      it('should handle login redirect with query parameters', () => {
        ...
        const encodedNext = encodeURIComponent('/read/test-doc?tab=glossary&highlight=consciousness')
        const searchParams = new URLSearchParams(`next=${encodedNext}`)
        const result = getRedirectUrl(searchParams, '/dashboard')

        expect(result).toBe('/read/test-doc?tab=glossary&highlight=consciousness')
      })

      it('should handle unencoded query parameters correctly', () => {
        const searchParams = new URLSearchParams('next=/read/test-doc?tab=glossary&highlight=consciousness')
        const result = getRedirectUrl(searchParams, '/dashboard')
        ...
        expect(result).toBe('/read/test-doc?tab=glossary')
      })
