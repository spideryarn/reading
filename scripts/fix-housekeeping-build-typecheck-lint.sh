...
exec claude -p --model opus --verbose --output-format stream-json --max-turns 25 --dangerously-skip-permissions "Please follow the comprehensive process outlined in the attached instruction file to systematically fix build, TypeScript, and linting issues. Work through Stage 1 (Assessment), Stage 2 (Resolution), and Stage 3 (Verification) as documented. Use subagents where appropriate to avoid context pollution. Focus on production code over test code. If you encounter complex/ambiguous decisions that need discussion, then skip them." < "$INSTRUCTION_FILE"
