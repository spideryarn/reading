...
- [x] Run `./scripts/sync-worktrees.ts` to ensure we have latest changes
...
- [ ] Create simplified version of `usePersistentChat` hook
  ...
  - [ ] Add separate `isLoading` state for subtle loading indicators
...
- [ ] Add refresh button to chat header in `assistant-chat.tsx`
  ...
  - [ ] Display "last refreshed" timestamp if helpful
...
- [ ] Update tests to reflect simplified approach
  - [ ] Update `usePersistentChat.test.ts`
  - [ ] Update `assistant-chat.test.tsx`
...
- [ ] Update `docs/CHATBOT_ASSISTANT_UI_INTEGRATION.md`
  - [ ] Update the "Database Persistence Patterns" section
  - [ ] Document the simplified approach
  - [ ] Remove references to ThreadHistoryAdapter
  - [ ] Add section on manual refresh pattern
- [ ] Update `docs/ASSISTANT_UI_DATABASE_PERSISTENCE.md` if it exists
- [ ] Add implementation notes about why we chose this approach

### Stage: Manual testing and review

- [ ] Test chat loads immediately on document page
- [ ] Test messages persist after page refresh
- [ ] Test refresh button updates conversation
- [ ] Test switching between documents
- [ ] Review with user for any issues or concerns
- [ ] Follow `docs/DEBRIEF_PROGRESS.md` to summarize
