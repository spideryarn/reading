...
- If there are multiple files, use tasks and subagents (provided with rich context)
...
- Search carefully for all the places that refer to each file, and update them appropriately.
  - Use **sd** for updating references across the codebase (see `docs/reference/SD_FIND_REPLACE.md`)
...
- Once you have finished, commit these changes as a single commit, following `docs/instructions/GIT_COMMIT_CHANGES.md`