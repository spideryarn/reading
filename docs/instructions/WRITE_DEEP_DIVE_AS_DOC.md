...
Then write this up as a detailed reference doc, following the instructions in `docs/instructions/WRITE_EVERGREEN_DOC.md`. Include url links/references (as well as mentions of our own code/docs etc), so we can track down the original sources later if we need to.