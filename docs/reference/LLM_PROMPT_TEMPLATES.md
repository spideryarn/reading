...
**All LLM calls now track token usage and costs automatically.** This system provides complete visibility into AI service consumption across all features.

### Enhanced Functions for Usage Tracking

The prompt template system provides enhanced functions that return both text and usage metadata:
...
The `usage` object contains detailed token consumption data:
...
interface PromptUsage {
  ...
  reasoningTokens?: number  // Additional tokens for reasoning (Claude thinking mode)
}
```

### Database Storage and Cost Calculation

All AI calls are tracked in the `ai_calls` table with:
- **Token counts**: `prompt_tokens`, `completion_tokens`, `total_tokens`, `reasoning_tokens`
- **Cost calculation**: Automatic cost calculation using current pricing data
...
```sql
-- Query total costs for a document
SELECT 
  d.title,
  COUNT(ac.id) as ai_calls_count,
  SUM(ac.prompt_tokens) as total_prompt_tokens,
  SUM(ac.completion_tokens) as total_completion_tokens,
  ROUND(
    SUM(ac.prompt_tokens * am.input_token_cost) + 
    SUM(ac.completion_tokens * am.output_token_cost), 
    6
  ) as total_cost_usd
FROM documents d
JOIN ai_calls ac ON ac.document_id = d.id
JOIN ai_models am ON am.model_id = ac.model_id AND am.provider = ac.provider
WHERE d.id = 'your-document-id'
GROUP BY d.id, d.title;
```

### API Route Implementation with Tracking

For new features, use this pattern to ensure proper usage tracking:
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Execute prompt with usage tracking
    const result = await executePromptWithUsage(myFeaturePrompt, {
      content: body.content,
      // ... other template variables
    })
    
    // Complete AI call with usage metadata
...
### Current Pricing Data (June 2025)

The system includes current pricing for all supported models:
...
- **2.5 Flash**: $0.10 input / $0.40 output per million tokens
- **2.5 Pro**: $1.25 input / $10.00 output per million tokens (≤200K tokens)

**Note**: Pricing data is stored in the `ai_models` table and can be updated via database migrations as rates change.
