...
- `docs/reference/TESTING_DATABASE.md` - Database testing patterns and shared database approach
- `docs/reference/TESTING_TROUBLESHOOTING.md` - Known issues and workarounds
- `docs/reference/TESTING_DATABASE_CLEANUP.md` - Test data cleanup utilities
...
1. **Create `.env.test`**: Copy your `.env.local` file to `.env.test`:
   ...
   **Important**: We use a **shared database approach** - `.env.test` should point to the same local Supabase instance as `.env.local`. See `docs/reference/TESTING_DATABASE.md` for details on our test isolation approach.
...
- **Database testing**: See `docs/reference/TESTING_DATABASE.md` for our shared database approach and test isolation patterns